...
def graph_indexing(graph):
    idx = int(graph["file"].split(".py")[0].split("/")[-1])
    del graph["file"]
...
def joern_parse(joern_path, input_path, output_path, file_name):
    ...
    joern_parse_call = subprocess.run([joern_path + "joern-parse", "--output", output_path + out_file,
                                       input_path],
...
def joern_create(joern_path, in_path, out_path, cpg_files):
    ...
    for cpg_file in cpg_files:
        ...
        if os.path.exists(in_path+cpg_file):
            ...
            script_path = f"{os.path.dirname(os.path.abspath(joern_path))}/joern-cli/graph-for-funcs2.sc"
